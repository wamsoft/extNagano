...
//
//   ■ 復元における簡略化・不確実点(重要) -----------------------------------
//   本効果は 32bit x87 FPU 命令を多用しており、Ghidra が ST0 上の中間値を
//   復元しきれていない箇所がある。以下は「構造・可逆な算術はバイナリに忠実、
//   数値の出所が追えない 2 点のみ妥当な式で近似」した実装である:
//     (1) 移動量テーブル MoveTable[t] の厳密な式(2次式係数のスケールと
//         bound によるハネ返り処理)。ドキュメントの 2 次式 + bound を
//         周期(剰余)として実装。BuildMoveTable のコメント参照。
//     (2) 移動画素を OutBuf へ合成する際の不透明度の指標。原コードは
//         StartProcess で作る不透明度テーブル(OpaTableA/B)を参照するが、
//         そのインデクスが復元できないため「開始時間でフェードイン」として
//         OpaTableA/B[starttime] を採用した。Scatter のコメント参照。
//   それ以外(ルール画素の分解, 方向の量子化(cos/sin の四捨五入), 移動量の
//   >>19 スケール, 3 通りの合成分岐, 領域転送)は復元元に忠実。
//---------------------------------------------------------------------------
...
public:
		  ...
		  Bound1(ClampI(bound1, 0, 0x7f)), Bound2(ClampI(bound2, 0, 0x7f))
	{
		...
		//   angle = i * 2π / 255。cos/sin を四捨五入するため実質 {-1,0,1} に
		//   量子化され、方向は 8 方位に丸まる(ドキュメントの 0:→/63:↓/
		//   127:←/191:↑ に一致)。
		...
		for(tjs_int i = 0; i < 256; i++)
		{
			...
			CosTable[i] = LRound(cos(angle)); // dx 符号
			SinTable[i] = LRound(sin(angle)); // dy 符号
		}
